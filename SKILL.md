...
description: Reads any webpage and generates valid JSON-LD structured data markup (schema.org) for it. Detects the right schema type automatically: FAQPage, Article, BlogPosting, Organization, Product, WebSite, HowTo, BreadcrumbList, SoftwareApplication, or LocalBusiness. Outputs a script tag ready to paste into your HTML, or opens a GitHub PR with the markup injected. Use when asked to add schema markup, add structured data, generate JSON-LD, improve SEO with schema, add rich results support, or add schema.org markup to a page. Trigger when a user mentions schema, JSON-LD, structured data, rich snippets, or wants to improve how their pages appear in Google search results.
